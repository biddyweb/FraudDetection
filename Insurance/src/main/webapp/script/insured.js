var app = angular.module('insureds', ['ngResource', 'ngGrid', 'ui.bootstrap','ngRoute','dynform']);

//Create a controller with name personsListController to bind to the grid section.
app.controller('personsListController', function ($scope, $rootScope, personService, $http, $route) {
	// Initialize required information: sorting, the first page to show and the grid options.
	$scope.sortInfo = {fields: ['id'], directions: ['asc']};
	$scope.insureds = {currentPage: 1};
	...
	$scope.gridOptions = {
			...
			columnDefs: [
			             ...
			             { field: 'insuranceNumber', displayName: 'CCN' },
			             { field: 'score', displayName: 'Score' },
			             { field: 'decision', displayName: 'Result' }
			             ],
	...
	$scope.refreshGrid = function () {
		var listPersonsArgs = {
				page: $scope.insureds.currentPage,
...
app.controller('personsFormController', function ($scope, $rootScope, personService) {
	...
	$scope.updatePerson = function () {
		personService.save($scope.insured).$promise.then(
				function () {
	...
	$scope.$on('personSelected', function (event, id) {
		$scope.insured = personService.get({id: id});
	});
...
app.factory('personService', function ($resource) {
	return $resource('resources/insureds/:id');
});
